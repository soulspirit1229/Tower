...
-(id) init {
    [super init];
    CCSprite *background =
    [CCSprite spriteWithFile:@"yellowlevelscene.png"];
    [background setPosition:ccp(winSize.width/2,
                                winSize.height/2)];
    [self addChild:background z:0];
    
    sceneType = kGameLevel5;
    return self;
...
-(void) createGround {
    groundblock = [CCSprite spriteWithSpriteFrameName:@"116.png"];
    groundblock.position=ccp(winSize.width/4, groundblock.contentSize.height/2);
    [self addChild:groundblock z:100];
    
    b2BodyDef groundblockBodyDef;
    groundblockBodyDef.position.Set(winSize.width/4/PTM_RATIO, groundblock.contentSize.height/2/PTM_RATIO);
...
-(void) createBlocks {
    NSArray *images = [NSArray arrayWithObjects:@"013.png", @"017.png", @"008.png", @"004.png", @"002.png",@"101.png" , nil];
